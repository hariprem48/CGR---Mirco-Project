"""
NIGHT RACER - Drive by Headlight
==================================
A Computer Graphics mini-project demonstrating 8 classic algorithms
inside one real, playable Pygame game.

STORY / GOAL
    You are racing at night on an oval track. Only the cone lit by your
    headlights is visible - the rest of the world is dark. Stay on the
    tarmac (grass slows you down a lot) and cross all three checkpoints
    to complete a lap.

CONTROLS
    UP / DOWN     -> accelerate / brake-reverse
    LEFT / RIGHT  -> steer
    L             -> toggle headlight-ray rendering between DDA and Bresenham
    M             -> toggle the little "explored track" minimap overlay
    R             -> reset car to the start line
    ESC / close window -> quit

ALGORITHM MAP
    1. dda_line                 -> draws each sampled headlight ray (default)
    2. bresenham_line           -> draws track walls, the barrier obstacle,
                                    checkpoints, and (if 'L' toggled) the rays
    3. cohen_sutherland_clip    -> final exact clip of every screen-space
                                    segment (rays & walls) to the window
    4. liang_barsky_clip        -> cheap broad-phase viewport-culling test:
                                    skip drawing a wall segment at all if its
                                    bounding box misses the padded viewport
    5. polygon_clip_line        -> (general/Cyrus-Beck) clips a headlight ray
                                    against the convex roadside barrier, so
                                    light realistically stops at the obstacle
    6. scanline_fill            -> fills the asphalt ring, the grass island,
                                    the barrier, and the final headlight cone
    7. flood_fill               -> (a) precomputes, once, a track/grass mask
                                    used every frame for the off-track speed
                                    penalty; (b) incrementally reveals a
                                    "discovered road" fog on the minimap
    8. sutherland_hodgman_clip  -> clips the raw headlight cone polygon
                                    against the local lane corridor (the
                                    quad formed by the nearest outer + inner
                                    wall segments) so the cone never bleeds
                                    through a track wall
"""
